...
Thirdly, the research shows that Copilot has the capabilities to save 75% (30 min experimental cut-off vs. 2 h current development time) of the time needed for writing unit-tests per story and yet maintain existing coverage rates (85% versus 82% using the "high"-strategy). This already adds financial value as the output can be enhanced making each hour more valuable. Simply calculating the hourly cost savings illustrates the economic benefit even better. GitHub Copilot Business costs 17,67€ per month, which can be broken down into hourly costs of eleven cents (GitHub, n.d.). The minimum salary is 13.13€/h based on the Austrian IT collective bargaining agreement (WKO, 2024). Therefore, the financial benefit is therefore 13,02€ per hour as saving 75% development time does not mean that Copilot remains inactive during the saved time but can be used for new tasks.
