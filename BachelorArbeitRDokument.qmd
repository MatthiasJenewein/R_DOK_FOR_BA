...
barplot(aggDataTime$Time,
        ...
        col = "lightblue",
        border = "black",
        main = "Bar Chart for Cycle Time - Strategy",
        xlab = "Prompting Strategy",
        ylab = "Average Cycle Time")
```
...
The barplot shows a clear trend regarding prompting strategies: The better the prompting strategy, the higher the test coverage.
...
barplot(aggDataTimeModel$Time,
        ...
        col = "lightblue",
        border = "black",
        main = "Bar Chart for Cycle Time - Model",
        xlab = "Model",
        ylab = "Average Cycle Time")
```
...
## Grouped Analysis - Time & Coverage
...
### Manova - Cycle Time & **Coverage**

Here, Manova (Multivaritive Anonv) evaluates whether the vector of means for Time and Coverage differs across the levels of the Model factor.
...
This might seem contrary to the analysis done in prior chapters and the choice of models might actually play a **minor** role. However, the manova model also shows that the prompting strategy as a predictor with a Pillai's trace of 0.15206, an approximate F-value of 5.4308 and a p-value of 0.0003234 compared to the model with a Pillai's trace of 0.076863, an approximate F-value of 2.6379 and a p-value of 0.03442 is significantly more important than the model. This indicates that the prompting strategy is far more important regarding the combination of cycle time and test coverage.
...
As mentioned at the beginning of this thesis, this is a vast topic and despite the efforts of this thesis, much needs still to explored. It will take a lot more and detailed analysis to truly find the "best prompting strategy". Especially, if thinkng about topics like complete automation (e.g. using the identified strategies to automatically trigger Copilot or different AI tools) , there needs to be a lot more testing and research done to enhance quality and stability of the model's output. This can be only reached by even more refined prompting techniques which are the connection between humans and AI models. This paper works more as a prototype, a concept to testify the importance of prompt engineering and works as a first guidelines what has to be considered and how the output of AI tools like GitHub Copilot can be enhanced.

To answer the secondary research questions proposed in Chapter 1:

Firstly, yes, AI tools like GitHub Copilot can positively affect test coverage and cycle time (cycle time used as a measurement to not only talk about performance but also financial standpoints). This becomes especially apparant, if you look at them in relation to each other. We might not have reached a potential yet, where test coverage rates above 90% can be achieved stably and steadily. However, already the research done in this thesis shows that AI can be a massive time saver and at the same reach consistent test coverage rates (that are at least as high as the current test coverage reached in the specific project at POI - which is about 85% compared to the 81% reached by the "high"-strategy).

Secondly, yes, this paper shows clearly that the prompting strategy matters and a sophisticated, advanced and clear prompting strategy positively influences cycle time and test coverage and therefore, it is recommended to opt for these strategies. The more AI will be integrated into existing processes, the more important these strategies will become (this is true for all domains, not just software engineering).
